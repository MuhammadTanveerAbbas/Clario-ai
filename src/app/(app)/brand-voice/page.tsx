...
function Skeleton({ w = "100%", h = 16, r = 8 }: { w?: string | number; h?: number; r?: number }) {
  return <div style={{ width: w, height: h, borderRadius: r, background: "linear-gradient(90deg, hsl(var(--card)) 25%, hsl(var(--border)) 50%, hsl(var(--card)) 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.5s infinite" }} />;
}
...
function ToastContainer({ toasts, dismiss }: { toasts: Toast[]; dismiss: (id: string) => void }) {
  return (
    <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 1000, display: "flex", flexDirection: "column", gap: 8 }}>
      {toasts.map(t => (
        <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 10, background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderLeft: `3px solid ${t.type === "success" ? "#10b981" : t.type === "error" ? "#ef4444" : "hsl(var(--accent))"}`, borderRadius: 10, padding: "11px 14px", boxShadow: "0 8px 24px rgba(0,0,0,.2)", animation: "fu .3s ease both", maxWidth: 320, fontFamily: "var(--sans)" }}>
...
export default function BrandVoicePage() {
  ...
  useEffect(() => {
    ...
    supabase.from("profiles").select("full_name, subscription_tier").eq("id", authUser.id).single().then(({ data }) => { if (data) setUserProfile(data); });
    loadVoices();
    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .voice-card{background:hsl(var(--card));border:1px solid var(--card-b);border-radius:14px;padding:20px;animation:fu .4s ease both}
        ...
        .modal{background:var(--bg2);border:1px solid hsl(var(--border));border-radius:18px;width:100%;max-width:520px;padding:24px;animation:fu .25s ease both;max-height:90vh;overflow-y:auto}
        ...
        .field-input{width:100%;background:var(--bg3);border:1px solid hsl(var(--border));border-radius:9px;padding:10px 12px;font-family:var(--sans);font-size:.86rem;color:var(--text);outline:none;transition:border-color .15s}
        .field-input:focus{border-color:hsl(var(--accent))}
      ...
      <div className="dash-layout">
        <div className="main-area">
          <div className="topbar">
            ...
            <button onClick={openCreate} style={{ background: "hsl(var(--accent))", color: "#fff", border: "none", borderRadius: 9, padding: "7px 16px", fontFamily: "var(--sans)", fontSize: ".82rem", fontWeight: 600, cursor: "pointer" }}>+ Create Voice</button>
          ...
          <div className="page-content" style={{ maxWidth: 860, margin: "0 auto" }}>
            ...
            {loading ? (
              <div className="voices-grid">
                {[1, 2].map(i => <div key={i} style={{ background: "hsl(var(--card))", border: "1px solid var(--card-b)", borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 12 }}><Skeleton h={20} w="60%" /><Skeleton h={12} /><Skeleton h={12} w="80%" /></div>)}
              </div>
            ) : voices.length === 0 ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "60px 20px", animation: "fu .4s ease both" }}>
                ...
                <button onClick={openCreate} style={{ background: "hsl(var(--accent))", color: "#fff", border: "none", borderRadius: 10, padding: "10px 24px", fontFamily: "var(--sans)", fontSize: ".88rem", fontWeight: 600, cursor: "pointer", marginBottom: 32 }}>Create your first voice</button>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, width: "100%", maxWidth: 560 }} className="feature-grid-3">
                  {[["Sounds like you", "Clario learns your tone, vocabulary, and personality"], ["Always on", "Activate a voice and it applies to every AI output automatically"], ["Multiple voices", "Create different voices for different brands or projects"]].map(([title, desc]) => (
                    <div key={title} style={{ background: "var(--bg3)", border: "1px solid hsl(var(--border))", borderRadius: 12, padding: 16 }}>
                ...
                {voices.map((v, i) => (
                  <div key={v.id} className="voice-card" style={{ animationDelay: `${i * 0.05}s` }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                      ...
                      <button onClick={() => openEdit(v)} style={{ background: "var(--bg3)", border: "1px solid hsl(var(--border))", borderRadius: 7, padding: "4px 10px", fontSize: ".72rem", color: "var(--text2)", cursor: "pointer" }}>Edit</button>
                    ...
                    {[["Tone", v.tone], ["Vocabulary", v.vocabulary], ["Personality", v.personality]].map(([label, val]) => val && (
                      <div key={label} style={{ marginBottom: 10 }}>
                        ...
                        <div style={{ height: 4, background: "var(--bg3)", borderRadius: 100, overflow: "hidden" }}>
                          <div style={{ height: "100%", width: "70%", background: "hsl(var(--accent))", borderRadius: 100 }} />
                        </div>
                      </div>
                    ))}
                    ...
                    {deleteConfirm === v.id && (
                      <div style={{ marginTop: 12, padding: 12, background: "rgba(239,68,68,.08)", border: "1px solid rgba(239,68,68,.2)", borderRadius: 10 }}>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          ...
                          <button onClick={() => setDeleteConfirm(null)} style={{ background: "var(--bg3)", border: "1px solid hsl(var(--border))", borderRadius: 7, padding: "6px 14px", fontSize: ".78rem", color: "var(--text2)", cursor: "pointer" }}>Cancel</button>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setModalOpen(false); }}>
          <div className="modal">
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
              <button onClick={() => setModalOpen(false)} style={{ background: "var(--bg3)", border: "1px solid hsl(var(--border))", borderRadius: 9, padding: "9px 18px", fontFamily: "var(--sans)", fontSize: ".84rem", color: "var(--text2)", cursor: "pointer" }}>Cancel</button>
              <button onClick={handleSave} disabled={saving} style={{ background: "hsl(var(--accent))", color: "#fff", border: "none", borderRadius: 9, padding: "9px 18px", fontFamily: "var(--sans)", fontSize: ".84rem", fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? .6 : 1 }}>{saving ? "Saving..." : "Save Voice"}</button>
            </div>
          </div>
        </div>
      )}
