...
export function SummarizerTool() {
  ...
  const handleExportMarkdown = async (): Promise<void> => {
    ...
    let text = summaryRef.current.innerText || summaryRef.current.textContent || '';
...
*Generated by Clario Summarizer - Transform text into clear, actionable summaries.*`;
    ...
    const filename = `Clario-Summary-${state.mode.replace(/\s+/g, '-')}-${timestamp}.md`;
    a.download = filename;
    ...
    toast({
      ...
      description: "High-quality Markdown file downloaded with clean formatting.",
    });
  };
              ...
              <Button
                ...
                onClick={() => {
                  toast({
                    ...
                    description: "Ctrl+Enter: Summarize • Ctrl+K: Copy • Ctrl+S: Export",
                  });
                }}
