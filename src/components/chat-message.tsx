              ...
              <div className="prose prose-invert prose-sm md:prose-base max-w-none">
                <ReactMarkdown
                  ...
                  components={{
                    ...
                    p: ({ node, ...props }: any) => (
                      <p
                        className="text-white mb-3 last:mb-0 leading-relaxed"
                        {...props}
                      />
                    ),
                    ul: ({ node, ...props }: any) => (
                      <ul
                        className="list-disc list-inside text-white space-y-2 my-2 ml-2"
                        {...props}
                      />
                    ),
                    ol: ({ node, ...props }: any) => (
                      <ol
                        className="list-decimal list-inside text-white space-y-2 my-2 ml-2"
                        {...props}
                      />
                    ),
                    li: ({ node, ...props }: any) => (
                    ...
                    h1: ({ node, ...props }: any) => (
                      <h1
                        className="text-xl md:text-2xl font-bold text-white mt-4 mb-3"
                        {...props}
                      />
                    ),
                    h2: ({ node, ...props }: any) => (
                      <h2
                        className="text-lg md:text-xl font-bold text-white mt-4 mb-2"
                        {...props}
                      />
                    ),
                    h3: ({ node, ...props }: any) => (
                      <h3
                        className="text-base md:text-lg font-bold text-white mt-3 mb-2"
                        {...props}
                      />
                    ),
                    blockquote: ({ node, ...props }: any) => (
                      <blockquote
                        className="border-l-4 border-blue-400 pl-4 italic text-gray-300 my-3 bg-white/5 py-2 rounded"
                        {...props}
                      />
                    ),
                  }}
